...
void StoreGpr(std::uint8_t *code, std::size_t &offset, std::uint8_t reg,
...
constexpr std::uint64_t ValidationStack[] = {0x7777'7777'7777'7777ull,
...
void *SysvThunkArena::Create(void *context, std::uint64_t slot,
                             SysvDispatch dispatch) {
  if (!dispatch)
    throw std::invalid_argument("SysV thunk sem dispatcher.");
  auto *page = static_cast<std::uint8_t *>(
      Core::PlatformMemory::Allocate(GetCurrentProcess(), nullptr, PageSize,
                                     MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE));
  if (!page)
    throw std::runtime_error(
        "Não foi possível reservar a página do thunk SysV.");
  ...
  Byte(page, offset, 0x48);
  Byte(page, offset, 0x8D);
  Byte(page, offset, 0x84);
  Byte(page, offset, 0x24);
  U32(page, offset, FrameSize); // lea rax,[rsp+FrameSize]
  ...
  Byte(page, offset, 0x48);
  Byte(page, offset, 0xB9);
  U64(page, offset,
      reinterpret_cast<std::uint64_t>(context)); // mov rcx,context
  Byte(page, offset, 0x48);
  Byte(page, offset, 0xBA);
  U64(page, offset, slot); // mov rdx,slot
  Byte(page, offset, 0x4C);
  Byte(page, offset, 0x8D);
  Byte(page, offset, 0x44);
  Byte(page, offset, 0x24);
  Byte(page, offset, WindowsShadowSpace); // lea r8,[rsp+20h]
  Byte(page, offset, 0x4C);
  Byte(page, offset, 0x8B);
  Byte(page, offset, 0x4C);
  Byte(page, offset, 0x24);
  Byte(page, offset, WindowsShadowSpace + 0x30); // mov r9,[rsp+50h]
  Byte(page, offset, 0x48);
  ...
  DWORD previous{};
  if (!Core::PlatformMemory::Protect(GetCurrentProcess(), page, PageSize,
  ...
  if (!FlushInstructionCache(GetCurrentProcess(), page, offset)) {
    Core::PlatformMemory::Free(GetCurrentProcess(), page, 0, MEM_RELEASE);
    throw std::runtime_error(
        "Não foi possível limpar o cache de instruções do thunk SysV.");
  }
...
SysvAbiValidation ValidateSysvThunkAbi() {
  ...
  if (!caller)
    throw std::runtime_error(
        "Não foi possível reservar o validador da ABI SysV.");
  ...
  for (std::size_t index = 0; index != 2; ++index) {
    ...
    U64(caller, offset, ValidationStack[index]);
    Byte(caller, offset, 0x48);
    Byte(caller, offset, 0x89);
    Byte(caller, offset, static_cast<std::uint8_t>(index == 0 ? 0x04 : 0x44));
    Byte(caller, offset, 0x24);
    if (index != 0)
      Byte(caller, offset, 0x08);
  }
  // SysV integer arguments: RDI, RSI, RDX, RCX, R8, R9.
  constexpr std::uint8_t movImmediatePrefixes[] = {0x48, 0x48, 0x48,
                                                   0x48, 0x49, 0x49};
  constexpr std::uint8_t movImmediateOpcodes[] = {0xBF, 0xBE, 0xBA,
                                                  0xB9, 0xB8, 0xB9};
  for (std::size_t index = 0; index != 6; ++index) {
  ...
  if (!Core::PlatformMemory::Protect(GetCurrentProcess(), caller, PageSize,
    ...
    throw std::runtime_error(
        "Não foi possível ativar o validador da ABI SysV.");
  }
...
std::uint64_t InvokeSysv2(void *entry, std::uint64_t argument0,
  ...
  Byte(caller, offset, 0x83);
  Byte(caller, offset, 0xEC);
  Byte(caller, offset, 0x28); // align the nested SysV call
  Byte(caller, offset, 0x48);
  Byte(caller, offset, 0xBF); // mov rdi,argument0
  U64(caller, offset, argument0);
  Byte(caller, offset, 0x48);
  Byte(caller, offset, 0xBE); // mov rsi,argument1
  U64(caller, offset, argument1);
  Byte(caller, offset, 0x48);
  Byte(caller, offset, 0xB8); // mov rax,entry
