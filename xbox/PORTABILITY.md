...
O único teste ativo compila o `src/core/loader/elf.cpp` original, executa
`Elf::LoadSegment` e chama o código carregado com um backend UWP derivado de `IFile`; os
...
| Formatos do núcleo | `common/endian.h`, `core/file_format/psf.h`, `psf.cpp`, `core/loader/elf.h`, `elf.cpp` e `core/file_sys/ifile.h` | `Elf::LoadSegment` original transfere código gerado pelo projeto, a página vira RX e a função retorna 42 | SELF real/descriptografia, relocação, ABI, TLS, exceções e fontes restantes com dependências de logging/assert |
