...
& git -C $root submodule update --init --depth 1 externals/miniz externals/freetype
if ($LASTEXITCODE -ne 0) { throw 'Pinned miniz checkout failed.' }
# miniz export header normally generated by CMake; static UWP build needs no export decoration.
Set-Content "$root\externals\miniz\miniz_export.h" "#pragma once`n#define MINIZ_EXPORT"
& cmake -S "$project\font" -B "$project\obj\fonts-uwp" -G 'Visual Studio 17 2022' -A x64 -DCMAKE_SYSTEM_NAME=WindowsStore "-DCMAKE_SYSTEM_VERSION=$SdkVersion"
if ($LASTEXITCODE -ne 0) { throw 'FreeType UWP configuration failed.' }
& cmake --build "$project\obj\fonts-uwp" --config Release --parallel
if ($LASTEXITCODE -ne 0) { throw 'FreeType UWP compilation failed.' }
& cmake -S "$project\font" -B "$project\obj\fonts-host" -G 'Visual Studio 17 2022' -A x64 -DXBOX_FONT_TESTS=ON
if ($LASTEXITCODE -ne 0) { throw 'Font ABI test configuration failed.' }
& cmake --build "$project\obj\fonts-host" --config Release --parallel
if ($LASTEXITCODE -ne 0) { throw 'Font ABI test compilation failed.' }
& ctest --test-dir "$project\obj\fonts-host" -C Release --output-on-failure
if ($LASTEXITCODE -ne 0) { throw 'Font rasterization / guest ABI test failed.' }
...
New-Item "$stage\Fonts" -ItemType Directory -Force | Out-Null
Copy-Item "$root\src\imgui\renderer\fonts\NotoSans-Regular.ttf", "$root\src\imgui\renderer\fonts\NotoSansCJK-Regular.ttc" "$stage\Fonts"
...
Copy-Item "$root\LICENSES\OFL-1.1.txt" "$stage\Licenses\Noto-OFL.txt"
Copy-Item "$root\externals\freetype\docs\GPLv2.TXT" "$stage\Licenses\FreeType-GPLv2.txt"
Copy-Item "$root\externals\freetype\LICENSE.TXT" "$stage\Licenses\FreeType.txt"
Copy-Item "$project\FONTS.md" "$stage\Licenses\font-notices.md"
