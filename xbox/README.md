...
A versão **0.49.0.0** inclui biblioteca horizontal inspirada no PS4, importação de
...
Os serviços iniciais de usuário e preferências do sistema agora escrevem por
ponteiros convidados validados: usuário inicial/login, nome `Xbox` e idioma,
data, hora, fuso, controle parental e botão de confirmação. O probe confere em
conjunto o usuário inicial e o idioma antes de liberar esse bloco para o runtime.
O subconjunto importado de `libSceRegMgr` mantém valores binários, strings e
inteiros em um registro isolado por processo, com limite de tamanho e validação
de ponteiros convidados. O probe confirma escrita e leitura binária pelo thunk.
O sistema de arquivos HLE expõe `/app0` somente para leitura e redireciona
`/data`, `/savedata0` e `/user` para `RuntimeData` dentro da instalação extraída.
As operações `open`, `close`, `read`, `write`, `lseek` e `fsync` rejeitam fuga por
`..` e passam somente buffers validados do mapa convidado.
As raízes confiáveis são normalizadas lexicalmente; o VFS não usa
`weakly_canonical`, cuja enumeração de diretórios é negada pelo AppContainer do Xbox.
As variantes POSIX de abertura, leitura, escrita, busca, sincronização e fechamento
usam o mesmo VFS. `access`, `mkdir`, `rmdir`, `rename`, `unlink`, `chmod` e `flock`
foram ligados ao armazenamento confinado; permissões Unix são aceitas como
compatibilidade sem ampliar as permissões reais do AppContainer.
...
A ponte de entrada de threads preserva também XMM6–XMM15, exigidos pelo ABI
