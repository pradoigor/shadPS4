// SPDX-FileCopyrightText: Copyright 2024 shadPS4 Emulator Project
// SPDX-License-Identifier: GPL-2.0-or-later
// RSA key material is ported from AzaharPlus/shadPS4Plus keys.h, commit 9d2e761.
// It is used only for the public FPKG key path; user-supplied keys remain supported.

#include "PkgBuiltinKeys.h"
#include <cstddef>
#include <string>

namespace Lab { namespace {
static constexpr unsigned char FakeKeyset_PublicExponent[4] = {
    0x00, 0x01, 0x00, 0x01,
};

static constexpr unsigned char FakeKeyset_Modulus[256] = {
    ,
};

static constexpr unsigned char FakeKeyset_Prime1[128] = {
    ,
};

static constexpr unsigned char FakeKeyset_Prime2[128] = {
    ,
};

static constexpr unsigned char FakeKeyset_Exponent1[128] = {
    ,
};

static constexpr unsigned char FakeKeyset_Exponent2[128] = {
    ,
};

static constexpr unsigned char FakeKeyset_Coefficient[128] = {
    ,
};

static constexpr unsigned char FakeKeyset_PrivateExponent[256] = {
    ,
};

static constexpr unsigned char PkgDerivedKey3Keyset_PublicExponent[4] = {
    0x00, 0x01, 0x00, 0x01,
};

static constexpr unsigned char PkgDerivedKey3Keyset_Modulus[256] = {
    ,
};

static constexpr unsigned char PkgDerivedKey3Keyset_Prime1[128] = {
    ,
};

static constexpr unsigned char PkgDerivedKey3Keyset_Prime2[128] = {
    ,
};

static constexpr unsigned char PkgDerivedKey3Keyset_Exponent1[128] = {
    ,
};

static constexpr unsigned char PkgDerivedKey3Keyset_Exponent2[128] = {
    ,
};

static constexpr unsigned char PkgDerivedKey3Keyset_Coefficient[128] = {
    ,
};

static constexpr unsigned char PkgDerivedKey3Keyset_PrivateExponent[256] = {
    ,
};

template <size_t N> Bytes Copy(const unsigned char (&value)[N]) { return Bytes(value, value + N); }
RsaFields MakeSet(const char* prefix) {
    RsaFields result;
    if (std::string(prefix) == "fake") {
        result.emplace("PublicExponent", Copy(FakeKeyset_PublicExponent));
        result.emplace("Modulus", Copy(FakeKeyset_Modulus));
        result.emplace("Prime1", Copy(FakeKeyset_Prime1));
        result.emplace("Prime2", Copy(FakeKeyset_Prime2));
        result.emplace("Exponent1", Copy(FakeKeyset_Exponent1));
        result.emplace("Exponent2", Copy(FakeKeyset_Exponent2));
        result.emplace("Coefficient", Copy(FakeKeyset_Coefficient));
        result.emplace("PrivateExponent", Copy(FakeKeyset_PrivateExponent));
    } else {
        result.emplace("PublicExponent", Copy(PkgDerivedKey3Keyset_PublicExponent));
        result.emplace("Modulus", Copy(PkgDerivedKey3Keyset_Modulus));
        result.emplace("Prime1", Copy(PkgDerivedKey3Keyset_Prime1));
        result.emplace("Prime2", Copy(PkgDerivedKey3Keyset_Prime2));
        result.emplace("Exponent1", Copy(PkgDerivedKey3Keyset_Exponent1));
        result.emplace("Exponent2", Copy(PkgDerivedKey3Keyset_Exponent2));
        result.emplace("Coefficient", Copy(PkgDerivedKey3Keyset_Coefficient));
        result.emplace("PrivateExponent", Copy(PkgDerivedKey3Keyset_PrivateExponent));
    }
    return result;
}
} // namespace
PackageKeys BuiltinFpkgKeys() { return {MakeSet("derived"), MakeSet("fake")}; }
} // namespace Lab

